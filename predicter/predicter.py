...
class BeamSearchNodeOptimizedCoverage(object):
    def __init__(self, dec_state, previous_node, dec_X, log_prob, length, eot_found, enc_valid_len, attention_weights):
        self.dec_state = dec_state
        self.previous_node = previous_node
        self.dec_X = dec_X
        self.log_prob = log_prob
        ...
        if beam_type == "beam":
            ...
            if self.previous_node is None:
                ...
                while current_node.previous_node is not None:
                    ...
                    coverage_penalization = [coverage_penalization[i]+attention_weights[i] for i in range(len(coverage_penalization))]
                    current_node = current_node.previous_node
                coverage_penalization = [log(min(coverage_penalization[i],1.0)+0.00001) for i in range(len(coverage_penalization))]
...
def batched_beam_decode_optimized_coverage(net, data_iter, num_steps, beam_size, eot_token,
    ...
    for batch in data_iter:
        ...
        for (enc_X, enc_valid_len) in zip(batched_enc_X, batched_enc_valid_len):
            ...
            dec_X = torch.unsqueeze(enc_X[:, -1, 0], 1)
            # Starting node
            initial_node = BeamSearchNodeOptimizedCoverage(dec_state, None, dec_X, 0, 0, False, enc_valid_len, None)
            ...
            for _ in range(num_steps):
                ...
                for j in range(beam_size):
                    ...
                    if attention_enabled:
                        node = BeamSearchNodeOptimizedCoverage(dec_state, current_node, current_idx,
                                                    current_node.log_prob + log(current_value), new_length,
                                                    current_node.eot_found or current_idx == eot_token, enc_valid_len, net.decoder.attention_weights)
                    else:
                        node = BeamSearchNodeOptimizedCoverage(dec_state, current_node, current_idx,
                                                    current_node.log_prob + log(current_value), new_length,
                                                    current_node.eot_found or current_idx == eot_token, enc_valid_len, None)
