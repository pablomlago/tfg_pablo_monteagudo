...
def masked_softmax(X, valid_lens, prepadding=False):
    ...
    if valid_lens is None:
        ...
        if prepadding:
            X = sequence_mask_prepadding(X.reshape(-1, shape[-1]), valid_lens,
                            value=-1e3)
        else:
...
def sequence_mask_prepadding(X, valid_len, value=0):
    """Mask irrelevant entries in sequences."""
    maxlen = X.size(1)
    mask = torch.arange((maxlen), dtype=torch.float32,
                        device=X.device)[None, :] < valid_len[:, None]
...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, valid_lens, prepadding=False):
        ...
        self.attention_weights = masked_softmax(scores, valid_lens, prepadding)
...
class Seq2SeqEncoderResourcePositionalPrePadding(d2l.Encoder):
    """The RNN encoder for sequence to sequence learning."""

    def __init__(self, num_activities, num_resources, embed_size, num_hiddens, time_features, num_layers,
                 dropout=0, **kwargs):
        super(Seq2SeqEncoderResourcePositionalPrePadding, self).__init__(**kwargs)
        # Embedding layer
        self.embed_size = embed_size
        self.time_features = time_features
        self.embedding_activity = nn.Embedding(num_activities, embed_size)
...
class Seq2SeqAttentionDecoderNoPositionalPrePadding(AttentionDecoder):
    def __init__(self, vocab_size, embed_size, num_hiddens, num_layers,
                 dropout=0, **kwargs):
        super(Seq2SeqAttentionDecoderNoPositionalPrePadding, self).__init__(**kwargs)
        self.attention = AdditiveAttention(num_hiddens, num_hiddens,
                                           num_hiddens, dropout)
        self.embed_size = embed_size
        self.embedding = nn.Embedding(vocab_size, embed_size)
...
class Seq2SeqEncoderResourcePositionalPrePaddingBidirectional(d2l.Encoder):
    """The RNN encoder for sequence to sequence learning."""

    def __init__(self, num_activities, num_resources, embed_size, num_hiddens, time_features, num_layers,
                 dropout=0, **kwargs):
        super(Seq2SeqEncoderResourcePositionalPrePaddingBidirectional, self).__init__(**kwargs)
        # Embedding layer
        self.embed_size = embed_size
        self.time_features = time_features
        self.embedding_activity = nn.Embedding(num_activities, embed_size)
...
class AdditiveAttentionBidirectional(nn.Module):
    """Additive attention."""

    def __init__(self, key_size, query_size, num_hiddens, dropout, **kwargs):
        super(AdditiveAttentionBidirectional, self).__init__(**kwargs)
        self.W_k = nn.Linear(key_size*2, num_hiddens*2, bias=False)
        self.W_q = nn.Linear(query_size*2, num_hiddens*2, bias=False)
        self.w_v = nn.Linear(num_hiddens*2, 1, bias=False)
        self.dropout = nn.Dropout(dropout)
...
class Seq2SeqAttentionDecoderNoPositionalPrePaddingBidirectional(AttentionDecoder):
    def __init__(self, vocab_size, embed_size, num_hiddens, num_layers,
                 dropout=0, **kwargs):
        super(Seq2SeqAttentionDecoderNoPositionalPrePaddingBidirectional, self).__init__(**kwargs)
        self.attention = AdditiveAttentionBidirectional(num_hiddens, num_hiddens,
                                           num_hiddens, dropout)
        self.embed_size = embed_size
        self.embedding = nn.Embedding(vocab_size, embed_size)
        self.rnn = nn.GRU(embed_size + num_hiddens*2, num_hiddens, num_layers,
                          dropout=dropout, bidirectional=True)
    ...
    def forward(self, X, state):
        ...
        for x in X:
            # Shape of `query`: (`batch_size`, 1, `num_hiddens`)
            query = torch.reshape(hidden_state, (int(hidden_state.size(dim=0)/2), -1 ,hidden_state.size(dim=2)*2))
            query = torch.unsqueeze(query[-1], dim=1)
            # Shape of `context`: (`batch_size`, 1, `num_hiddens`)
            context = self.attention(query, enc_outputs, enc_outputs,
                                     enc_valid_lens, True)
            # Concatenate on the feature dimension
