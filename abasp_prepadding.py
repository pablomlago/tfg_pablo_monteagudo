    ...
    if "beam" in postprocessing_type:
        for j in beam_width_to_try:
            ...
            if num_activities + 2 < j:
                ...
                continue
            curr_beam_width = j
            predictions = batched_beam_decode_optimized_prepadding(net, val_iter, num_steps, curr_beam_width, num_activities+2, device, name, not args.disable_attention, postprocessing_type=postprocessing_type)
...
print("Testing...")
if "beam" in postprocessing_type:
    predictions = batched_beam_decode_optimized_prepadding(net, test_iter, num_steps, beam_width, num_activities+2, device, name, not args.disable_attention, postprocessing_type=postprocessing_type)
else:
    predictions = predict_seq2seq_optimized_prepadding(net, test_iter, num_steps, device, name, not args.disable_attention, batch_size, postprocessing_strategy=postprocessing_type)
