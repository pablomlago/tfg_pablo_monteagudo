...
def __label_encode_activity(df_train, df_val, df_test, activity_col):
    ...
    df_train[activity_col] = encoder.fit_transform(df_train[[activity_col]])
    df_train[activity_col] = df_train[activity_col].apply(lambda x : 1 if x == -1 else x+2)
    #We apply the encoder over our validation and test set
    df_val[activity_col] = encoder.transform(df_val[[activity_col]])
    df_val[activity_col] = df_val[activity_col].apply(lambda x : 1 if x == -1 else x+2)
    df_test[activity_col] = encoder.transform(df_test[[activity_col]])
    df_test[activity_col] = df_test[activity_col].apply(lambda x : 1 if x == -1 else x+2)
...
def load_dataset_seq_seq_time_fold_csv(dataset, num_folds):
    ...
    for i in range(num_folds):  
        ...
        df_train, df_val, df_test, num_activities = __label_encode_activity(fold_train, fold_val, fold_test, dataset_config['activity_col'])
